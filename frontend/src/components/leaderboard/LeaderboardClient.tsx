  ...
  return (
    ...
    >
      ...
      <div className="mt-8 text-center z-10 relative">
        <Link to={`/profile/${user.username}`} className="text-xl text-white hover:text-[#00e5ff] transition-colors" style={{ fontFamily: "'Chakra Petch', sans-serif", fontWeight: 700 }}>
          {user.username}
        </Link>
    ...
    >
      <div className="grid sm:grid-cols-[auto_1fr_auto_auto_auto] gap-4 px-6 py-4 items-center">
        ...
        <div className="flex items-center gap-3">
          <Link to={`/profile/${user.username}`} className="text-white group-hover/row:text-[#00e5ff] transition-colors" onClick={(e) => e.stopPropagation()} style={{ fontFamily: "'Chakra Petch', sans-serif", fontWeight: 600 }}>
            {user.username}
          </Link>
        </div>
