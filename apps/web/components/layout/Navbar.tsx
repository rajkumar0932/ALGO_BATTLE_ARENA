...
export function Navbar() {
  ...
  return (
    <nav className={`sticky top-0 inset-x-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2 group">
            ...
            <span className="text-xl font-black text-white tracking-tight" style={{ fontFamily: "'Chakra Petch', sans-serif" }}>Algo<span className="text-accent-cyan" style={{ textShadow: '0 0 12px rgba(0,229,255,0.5)' }}>Battle</span></span>
          </Link>
              ...
              return (
                <Link
                  ...
                  className={`relative flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all duration-200 group ${
                    isActive ? "text-accent-cyan" : "text-gray-400 hover:text-gray-100 hover:bg-bg-hover"
                  }`}
                ...
                >
                  ...
                  {link.label}
                  {isActive && (
                    <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-4 h-[3px] rounded-full" style={{ background: '#00e5ff', boxShadow: '0 0 8px #00e5ff' }} />
                  )}
                </Link>
              );
            })}
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <Link href="/battle" className="group relative overflow-hidden hidden lg:flex items-center gap-2 px-6 py-2 text-sm font-bold rounded-lg text-[#080c18] transition-all hover:shadow-[0_0_20px_rgba(0,229,255,0.3)]" style={{ background: 'linear-gradient(135deg, #00e5ff, #0070f3)' }}>
              <div 
              ...
              <>
                ...
                <Link
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all duration-200 ${
                    pathname === "/dashboard" ? "text-accent-purple bg-accent-purple/10 border border-accent-purple/20" : "text-gray-400 hover:text-gray-100 hover:bg-bg-hover"
                  }`}
                >
      ...
      {/* Mobile menu (Sliding out from right) */}
      <div className={`fixed inset-y-0 right-0 w-64 bg-bg-card border-l border-border shadow-2xl transform transition-transform duration-300 ease-in-out md:hidden z-50 ${
        ...
        <div className="p-6 h-full flex flex-col">
          ...
          <Link href="/battle" className="btn-primary flex items-center justify-center gap-2 mb-8 py-3" onClick={() => setMobileOpen(false)}>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg font-bold transition-all ${
                    isActive ? "bg-accent-cyan/10 text-accent-cyan border border-accent-cyan/20" : "text-gray-400 hover:text-gray-100 hover:bg-bg-hover"
                  }`}
            ...
            {session ? (
              <div className="flex flex-col gap-2">
                <Link
                  href="/dashboard"
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg font-bold transition-all ${
                    pathname === "/dashboard" ? "bg-accent-purple/10 text-accent-purple border border-accent-purple/20" : "text-gray-400 hover:text-gray-100 hover:bg-bg-hover"
                  }`}
                  onClick={() => setMobileOpen(false)}
                >
